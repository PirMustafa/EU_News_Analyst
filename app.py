...
def load_data():
    ...
    try:
        ...
        return None, [], [], {}, "Offline: items.json is missing. Run build_index.py."
    except json.JSONDecodeError:
        logger.exception("JSON metadata file is corrupt or unreadable.")
        return None, [], [], {}, "Offline: items.json is corrupt or unreadable (invalid JSON)."
    except (OSError, UnicodeError):
        logger.exception("JSON metadata file is corrupt or unreadable.")
        return None, [], [], {}, "Offline: items.json is corrupt or unreadable."

    if not isinstance(items_data, list):
        logger.error("Embedding metadata file did not contain a list of items.")
        return None, [], [], {}, "Offline: items.json has an invalid structure. Run build_index.py."
